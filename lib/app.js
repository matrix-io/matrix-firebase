...
module.exports = {
  ...
  list: function (cb) {
    if (_.isArray(refs.userApps)) {
      async.map(refs.userApps, function (ref, callback) {
        // get deviceId
        var refArr = ref.toString().split('/');
        var deviceId = refArr[refArr.length-2];

        ref.on('value', function (data) {
          debug('app.list>', data.val());
          callback(null, { [deviceId]: data.val()});
        }); 
      }, function (err, results) {
        if (err) return cb(err);
        else return cb(null, results);
  ...
  getConfig: function (deviceId, appId, cb) {
    ...
    refs.base.ref('deviceapps/'+deviceId).child(appId)
      .child('public/config')
      .once('value', function (data) {
  ...
  setConfigKey: function (deviceId, appId, key, value, cb) {
    ...
    debug('config>', deviceId, key, value);
  ...
  watchNamedUserApp: function (deviceId, appName, cb) {
    ...
    refs.userDevices.child(deviceId + "/apps").orderByChild('name').equalTo(appName).limitToFirst(1).on('value', function (app) {
      if (app.exists()) {
      ...
      async.map(refs.userApps, function (ref, callback) {
        ...
        ref.orderByChild('name')
          ...
          .once('value', function (data) {
            ...
            if (data.exists()) {
              return callback(null, { [deviceId]: _.keys(data.val())[0] });
            } 
            else {
  ...
  getStatus: function (deviceId, appId, cb) {
    ...
    if(_.isArray(appId) && appId.length > 1){

      async.map(appId, function(data, callback){
        refs.base.ref('deviceapps/' + _.keys(data)[0])
        .child(_.values(data)[0])
        .child('public/runtime/status')
        .once('value', function (resp) {
          if (resp.exists()) {            
            return callback(null, resp.val());
          } else {
      ...
      refs.base.ref('deviceapps/' + _.keys(appId[0])[0])
      .child(_.values(appId[0])[0])
      .child('public/runtime/status')
      .once('value', function (resp) {
        if (resp.exists()) {
          cb(resp.val());
