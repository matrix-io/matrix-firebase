...
module.exports = {
  ...
  //update device status via Status update worker
  updateStatus: function (deviceToken, events ) {
    var options = {
      token: deviceToken,
      online: true
    };

    require('./helpers').updateStatus(options, events);
  ...
  watchDeviceOnDisconnect: function( deviceToken, cb ){
    console.log('init');
    var disconnectTaskId = refs.queue.push().key;
    refs.queue.child(disconnectTaskId).onDisconnect().update({
