...
var setDisconnection = require('./queue').setDisconnection;
...
module.exports = {
  ...
  startAppUnpublish: function(options, events) {
    userToken = require('./util').token;
    options._state = specs.application_unpublish.start_state;
    options.token = userToken;
    processTask(specs.application_unpublish, options, events);
  }
  ...
  statusUpdate: function(options){
    deviceToken = require('./util').token;
    options._state = specs.status_update.start_state;
    options.token = deviceToken;
    processTask(specs.status_update, options);
  },
  setDisconnection : function(options){
