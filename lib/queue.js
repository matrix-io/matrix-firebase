...
var specs = {
  ...
  application_unpublish: {
    error_state: 'error',
    finished_state: 'application-unpublish-completed',
    in_progress_state: 'application-unpublish-in-progress',
    retries: 3,
    start_state: 'application-unpublish',
    timeout: 10000
  },
  user_update: {
    error_state: 'error',
...
//set ondisconnection firebase over any specific node
var onDisconnect = function (options) {

  options.token = require('./util').token;
  var disconnectTaskId = refs.queue.push().key;

  refs.queue.child(disconnectTaskId).onDisconnect().update(options);
}

module.exports = {
  specs: specs,
  processTask: processTask,
  setDisconnection: onDisconnect
