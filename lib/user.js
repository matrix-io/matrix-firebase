...
module.exports = {
   ...
   * Group Support
   */
  setGroup: function(key, devices, cb) {
    if (_.isFunction(devices)) {
      cb = devices;
      devices = undefined;
    };

    if (_.isUndefined(devices)) { // Just create group without any device.
      refs.userGroups.child(key).set(false, function(err) {
        if (!err) {
          debug('Empty group created: ', key);
          cb();
        }
      });
    } else {
      refs.userGroups.child(key).set(devices, () => cb());
    }
  },
  removeGroup: function(key, cb) {
    refs.userGroups.child(key).set(null, (err) => {
      if (!err) {
        debug('Group deleted: ', key);
        cb();
      }
    });
  },
  getUserGroups: function(cb) {
    refs.userGroups.once('value')
    .then((snapshot) => {
