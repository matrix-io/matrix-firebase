...
function getAllApps(deviceId, cb) {
  appKeys = {};
  dids = _.isArray(deviceId) ? deviceId : [deviceId];

  // get apps from all devices
  async.each(dids, (did, done) => {
    firebaseApp.database().ref('users/'+uid+'/devices/'+did+'/apps').orderByKey().once('value', function (data) {
      // debug('app.getAll>', _.map(data.val(), 'name') );
      // store for use later
  
      if (data.exists()) {
  
        module.exports.records.apps = _.assign(module.exports.records.apps, data.val());
        var appMap = data.val();
  
        async.eachOf(appMap, function (meta, appId, callback) {
          // debug(appId, meta.name);
          var fbApp = firebaseApp.database().ref('deviceapps/' + [did, appId].join('/') + '/public');
  
          firebaseInstalls[did] = {};
          firebaseDeviceAppRefMap[appId] = fbApp;
          firebaseInstalls[did][appId] = fbApp;
...
module.exports = {
  init: function (userId, deviceId, token, env, cb) {
    ...
    async.doUntil(function () {
      firebaseApp.auth().signInWithCustomToken(token).then(function (user) {
        ...
        firebaseUserGroupsRef = firebaseApp.database().ref('users/' + userId + '/groups');
        firebaseQueueRef = firebaseApp.database().ref('queue/tasks');
        
        if (_.isArray(deviceId)) {
          firebaseDeviceRef = (did) => firebaseApp.database().ref('devices/' + did + '/public');
          firebaseUserAppsRef = (did) => firebaseApp.database().ref('users/' + userId + '/devices/' + did + '/apps');
          firebaseDeviceAppsRef = (did) => firebaseApp.database().ref('deviceapps/' + did);
        }
        // if deviceId is not an Array, keep using the same deviceId passed in init
        else {
        ...
        module.exports.refs.userGroups = firebaseUserGroupsRef;
    ...
    }, async.ensureAsync(checkConnection()));
  },
  ...
  refs: {
    ...
    // users/$userId/groups/
    userGroups: firebaseUserGroupsRef,
    // queue/tasks
